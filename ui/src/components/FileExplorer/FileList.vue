...
<template>
  ...
  <div v-else-if="firstRender" class="loading">
    Silakan pilih folder terlebih dahulu
  </div>

  <div v-else-if="!files" class="loading">
    Folder masih kosong
  </div>
    ...
    >
      ...
      <div class="file-actions" v-if="actions">
        <span
          ...
          @click="openModal(file, 'rename-file')"
          title="Edit file"
        >✏️</span>
        <span
          data-testid="delete-btn"
          @click="openModal(file, 'delete-file')"
          title="Hapus file"
        >🗑️</span>
      </div>
    </div>
